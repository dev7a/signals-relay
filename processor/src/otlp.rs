...
mod tests {
    ...
    fn decode_request(record: Value) -> ExportTraceServiceRequest {
        let bytes = convert_span_to_otlp_protobuf(record).expect("record should convert");
        ExportTraceServiceRequest::decode(bytes.as_slice()).expect("request should decode")
    }
    ...
    fn test_convert_span_to_otlp_protobuf_missing_endtime() {
        ...
        assert!(convert_span_to_otlp_protobuf(span).is_err());
    }
    ...
    fn test_convert_span_to_otlp_protobuf_complete() {
        ...
        let request = decode_request(span);
    ...
    fn test_converts_nested_attributes_to_kvlist() {
        let request = decode_request(linkable_target_fixture());
        let span = &request.resource_spans[0].scope_spans[0].spans[0];
    ...
    fn test_preserves_flags_from_input_span() {
        let request = decode_request(client_span_fixture());
        let span = &request.resource_spans[0].scope_spans[0].spans[0];
    ...
    fn test_converts_links_from_merged_record() {
        ...
        let request = decode_request(target);
        let span = &request.resource_spans[0].scope_spans[0].spans[0];
